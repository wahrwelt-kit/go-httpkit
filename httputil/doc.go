...
// # SSE and health
//
// NewSSEWriter and NewSSEWriterWithLimit return an SSEWriter for Server-Sent Events; Send and SendJSON are concurrent-safe. Close marks the writer done. NewSSEWriterWithLimit accepts MaxEventBytes(n) to cap event payload size (default 64KB when n is 0 or not set); larger payloads return ErrSSEPayloadTooLarge. To remove the limit pass math.MaxInt64. Heartbeat sends an SSE comment every interval to keep the connection alive through idle-timeout proxies; run it in a goroutine and it stops on ctx cancellation or Close
// HealthHandler runs Checker implementations in parallel with a configurable timeout (default 5s; use HealthTimeout to override) and returns JSON status and checks. Use HealthOnEncodeError to log or handle encode failures; HealthHideDetails omits check details from the response
package httputil
